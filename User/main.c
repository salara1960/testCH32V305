...
//
    ...
    while (queFlag) {
        ...
        case crcEvt: {
            ...
            char stz[16] = {0};
            if (gImage_test)
                sprintf (stz, " PIC=0x%04X", crc_pic);
            if (crc_calc) {
                crc_calc = false;
        ...
        case adcEvt:
            ...
            if (Get_ADC_Average (adc_channel, &ADC_val)) {
                ...
                if (!adc_mode)
                    temp = TempSensor_Volt_To_Temper (vcc);
        ...
        case sensReadyEvt:
            ...
            if ((lsens.temp != sens.temp) ||
                ...
                if (sid == BME280_SENSOR)
                    sprintf (screen + strlen (screen), " | %lu%%", lroundf (lsens.humi));
                LCD_WriteString (8,
